...
boolean RV3028::begin(TwoWire &wirePort)
{
	...
	uint8_t sensorPartNumber = readRegister(RV3028_ID0);
	if (sensorPartNumber != RV3028_PART_NUMBER_UPPER) //HW version for RV3028
...
void RV3028::reset(void)
{
	writeRegister(RV3028_CONF_KEY, RV3028_CONF_RST); //Writes reset value from datasheet	
}
...
bool RV3028::setTime(uint8_t hund, uint8_t sec, uint8_t min, uint8_t hour, uint8_t date, uint8_t month, uint16_t year, uint8_t day)
{
...
{
	if (readMultipleRegisters(RV3028_HUNDREDTHS, _time, TIME_ARRAY_LENGTH) == false)
...
{
    uint8_t value;
    value = readRegister(RV3028_SLP_CTRL);
    value |= (1 << 7);
    writeRegister(RV3028_SLP_CTRL, value);
}
 
void RV3028::setPowerSwitchFunction(uint8_t function)
{
    uint8_t value;
    value = readRegister(RV3028_CTRL2);
    value &= 0b11000011; // Clear PSWS bits
    value |= (function << PSWS_OFFSET);
    writeRegister(RV3028_CTRL2, value);
}
 
void RV3028::setPowerSwitchLock(bool lock)
{
    uint8_t value;
    value = readRegister(RV3028_OSC_STATUS);
    value &= ~(1 << 5);
    value |= (lock << 5);
    writeRegister(RV3028_OSC_STATUS, value);
}
 
void RV3028::setStaticPowerSwitchOutput(bool psw)
{
    uint8_t value;
    value = readRegister(RV3028_CTRL1);
    value &= ~(1 << CTRL1_PSWB);
...
void RV3028::enableLowPower()
{
	...
	writeRegister(RV3028_CONF_KEY, RV3028_CONF_WRT); //Write the correct value to CONFKEY to unlock this bank of registers
	writeRegister(RV3028_IOBATMODE, 0x00); //Disable I2C when on backup power

	writeRegister(RV3028_CONF_KEY, RV3028_CONF_WRT); //Unlock again
	writeRegister(RV3028_OUT_CTRL, 0x30); //Disable WDI input, Set bit 4, Disable RST in sleep, Disable CLK/INT in sleep

	writeRegister(RV3028_CONF_KEY, RV3028_CONF_OSC); //Unlock again
	writeRegister(RV3028_OSC_CTRL, 0b11111100); //OSEL=1, ACAL=11, BOS=1, FOS=1, IOPW=1, OFIE=0, ACIE=0	
...
bool RV3028::checkBattery(uint8_t voltage)
{
	setReferenceVoltage(voltage);
	uint8_t status = readRegister(RV3028_ANLG_STAT);
	if (status >= 0x80)
		return true;
...
void RV3028::setReferenceVoltage(uint8_t voltage)
{
	...
	writeRegister(RV3028_CONF_KEY, RV3028_CONF_WRT);
	writeRegister(RV3028_BREF_CTRL, value);
}

void RV3028::setEdgeTrigger(bool edgeTrigger)
{
	uint8_t value;
	value = readRegister(RV3028_RAM_EXT);
